...
if(!empty($_POST) && $_POST['Action']=='login_form'){
    ...
    if(mysqli_num_rows($result)==1){
        ...
        $Return['result'] = $_SESSION['UserData'] = array('user_id'=>$row['user_id'], 'status'=>$row['status']);
    ...
    /*Return*/
    output($Return);
    $details = mysqli_query($con, "SELECT name FROM tbl_user_profile WHERE email='$email' LIMIT 1");
    $row1= mysqli_fetch_assoc($details);
    $Return1['result1'] = $_SESSION['UserData1']= array('name' => $row1['name']);
